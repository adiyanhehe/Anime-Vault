...
Auto updates require the release assets generated by `electron-builder` plus their latest metadata files (`latest.yml`, `latest-mac.yml`, and `latest-linux.yml`) to be published to GitHub Releases. The workflow handles that for tagged builds and for `main` builds by resolving the release tag from `package.json` (for example, version `0.1.0` publishes to `v0.1.0`), creating and pushing that tag if it does not exist, and uploading with `gh release upload --clobber` so existing installer files are replaced instead of failing the release job.
