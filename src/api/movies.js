...
/**
 * Fetch latest episodes added – placeholder implementation
 */
export async function fetchLatestEpisodes(page = 1) {
  // The actual episode source is not yet defined; return an empty array for now.
  return [];
}
 ...
 * Includes a fallback that generates synthetic episode data for series.
 */
export async function fetchMediaMeta(type, imdbId) {
  ...
  try {
    ...
    console.warn('Stremio metadata fetch failed or timed out, generating fallback:', err);
  } finally {
    clearTimeout(timeoutId);
  }
...
// Note: The iframe‑based embed helpers have been moved to their own module.
